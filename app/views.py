      ...
      if 'username1' in request.session:
        ...
        try:
            ...
            if  var.name == "Technical_ability":
                ...
                if int(cnt) <= j:
                    vars = question.objects.filter(ctgry_id=var.id, dept_id = username4).order_by('?')[0:int(cnt)]
                    return render(request,'aptitude_start.html',{'vars': vars,'username': username,'var': var, 'mem':mem})
                else:
                    return redirect('aptitude_catagory')
            else:
                if int(cnt) <= j:
                    vars = question.objects.filter(ctgry_id=var.id).order_by('?')[0:int(cnt)]
                    return render(request,'aptitude_start.html',{'vars': vars,'username': username,'var': var, 'mem':mem})
                else:
                      return redirect('aptitude_catagory')
      else:
...
def submit(request,id):
    ...
    if 'username1' in request.session:
        ...
        if request.method == 'POST':
            ...
            mm.save()
            lst_cat=list(time_out.objects.filter(user_id=username1).values_list('category_id',flat=True))
           
            m = catagory.objects.exclude(id__in=lst_cat).values()
            msg_warning = "This section completed"
            return render(request,'aptitude_catagory.html',{'msg_warning':msg_warning,'username': username, 'm': m})
        else:
            return redirect('aptitude_catagory')
...
def admin_category_waiting(request,id):
      var = login.objects.get(id=id)
      mem = User.objects.all()
      m=candidates.objects.filter(reference= var.fullname, replay_status=3).order_by('-id')
...
def hr_category_newlist(request):
 if 'usernamehr2' in request.session:
      ...
      mem = login.objects.filter(
        designation_id=usernamehr) .filter(fullname=usernamehr1)   
      m=candidates.objects.filter(reference=usernamehr1) .filter(contact_status=0).order_by('-id')
...
def hr_category_contactedlist(request):
 if 'usernamehr2' in request.session:
      ...
      mem = login.objects.filter(
        designation_id=usernamehr) .filter(fullname=usernamehr1)   
      m=candidates.objects.filter(reference=usernamehr1,contact_status=1).order_by('-id')
...
def hr_category_intrestedlist(request):
 if 'usernamehr2' in request.session:
      ...
      mem = login.objects.filter(
        designation_id=usernamehr) .filter(fullname=usernamehr1)   
      #m=candidates.objects.filter(reference=usernamehr1)
      m=candidates.objects.filter(reference=usernamehr1,replay_status=1).order_by('-id')
...
def hr_category_rejectedlist(request):
 if 'usernamehr2' in request.session:
      ...
      mem = login.objects.filter(
        designation_id=usernamehr) .filter(fullname=usernamehr1)   
      #m=candidates.objects.filter(reference=usernamehr1)
      m=candidates.objects.filter(reference=usernamehr1,replay_status=2).order_by('-id')
...
def hr_category_history(request):
 if 'usernamehr2' in request.session:
      ...
      mem = login.objects.filter(
        designation_id=usernamehr) .filter(fullname=usernamehr1)   
      m=candidates.objects.filter(reference=usernamehr1).exclude(replay_status=0).order_by('-id')
...
def hr_passwordchange(request):
    if 'usernamehr2' in request.session:
      ...
      if request.session.has_key('usernamehr1'):
        ...
        usernamehr1 = "dummy"
      mem = login.objects.filter(designation_id=usernamehr) .filter(fullname=usernamehr1)
      if request.method=='POST':
